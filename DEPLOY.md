...
后台入口为 `/admin/`，文章、配置、导航和资源管理默认使用 GitHub Contents API。

- 静态 Nginx 部署：后台会在当前浏览器会话中使用用户填写的 GitHub Token 直连 GitHub；Token 只保存于 `sessionStorage`，退出后台或关闭标签页后清除。
- 服务端部署：建议启用 Astro server adapter，并将 GitHub Token、后台认证密钥放入服务端环境变量，再关闭浏览器直连模式。
- GitHub Token 至少需要目标仓库内容的读写权限；不要把 Token 写入配置文件、提交到仓库或复制到截图中。
- 静态构建无法提供真正的 HttpOnly 服务端会话。需要严格服务端认证时，不能继续使用纯 `dist/` + Nginx 托管模式。

后台改动完成后必须重新构建并部署 `dist/`，GitHub 中的文章和配置提交也需要下一次构建才能反映到静态页面。
