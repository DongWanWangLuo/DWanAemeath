...
export const getStoredSakuraEnabled = (): boolean => getBool("sakura_enabled", sakuraConfig.enable);
export const setStoredSakuraEnabled = (v: boolean): void => { setBool("sakura_enabled", v); window.dispatchEvent(new CustomEvent("dw_sakura_toggle", { detail: { enabled: v } })); };
export const getStoredWavesEnabled = (): boolean => getBool("waves_enabled", typeof backgroundWallpaper.common?.waves?.enable === "object" ? backgroundWallpaper.common.waves.enable.desktop : true);
export const setStoredWavesEnabled = (v: boolean): void => { setBool("waves_enabled", v); document.documentElement.setAttribute("data-waves-enabled", String(v)); };
export const getStoredGradientEnabled = (): boolean => getBool("gradient_enabled", typeof backgroundWallpaper.common?.gradient?.enable === "object" ? backgroundWallpaper.common.gradient.enable.desktop : true);
export const setStoredGradientEnabled = (v: boolean): void => { setBool("gradient_enabled", v); document.documentElement.setAttribute("data-gradient-enabled", String(v)); };
export const getStoredBannerTitleEnabled = (): boolean => getBool("banner_title_enabled", backgroundWallpaper.common?.homeText?.enable ?? true);
export const setStoredBannerTitleEnabled = (v: boolean): void => {
...
export const getStoredCarouselEnabled = (): boolean => getBool("carousel_enabled", backgroundWallpaper.common?.carousel?.enable ?? false);
export const setStoredCarouselEnabled = (v: boolean): void => { setBool("carousel_enabled", v); document.documentElement.setAttribute("data-banner-carousel-enabled", String(v)); };

export const getStoredWallpaperMode = (): WALLPAPER_MODE => {
  const s = localStorage.getItem(LK("wallpaper_mode")) as WALLPAPER_MODE | null;
  return s || "fullscreen";
};
export const setStoredWallpaperMode = (v: WALLPAPER_MODE): void => {
...
export const setStoredTheme = (v: "light" | "dark" | "system"): void => {
...
export const setStoredHue = (v: number): void => {
  localStorage.setItem(LK("hue"), String(v));
  document.documentElement.style.setProperty("--hue", String(v));
};

export const getStoredPostLayout = (): string => localStorage.getItem(LK("post_layout")) || "grid";
export const setStoredPostLayout = (v: string): void => localStorage.setItem(LK("post_layout"), v);
export const getStoredCategoryBar = (): boolean => getBool("category_bar", true);
export const setStoredCategoryBar = (v: boolean): void => setBool("category_bar", v);
export const getStoredShowTags = (): boolean => getBool("show_tags", true);
export const setStoredShowTags = (v: boolean): void => setBool("show_tags", v);
...
export function getSyncStatus(): { syncing: boolean; lastSync: string | null; error: string | null } {
  return {
...
export function setSyncing(on: boolean): void { localStorage.setItem(LK("syncing"), on ? "1" : "0"); }
export function setLastSync(ts: string): void { localStorage.setItem(LK("last_sync"), ts); }
export function setSyncError(err: string | null): void { localStorage.setItem(LK("sync_error"), err || ""); }
...
export function postToMarkdown(post: DraftPost): string {
  ...
  if (post.image) fm.push("image: " + JSON.stringify(post.image));
